...
public class ServerListEntryNormalPack implements GuiListExtended.IGuiListEntry
{
    ...
    protected ServerListEntryNormalPack(GuiMultiplayerPack ownerIn, ServerData serverIn)
    {
        this.owner = ownerIn;
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks)
    {
        if (!this.server.pinged)
        {
            ...
            EXECUTOR.submit(new Runnable()
            {
                public void run()
                {
                    try
                    {
                        ServerListEntryNormalPack.this.owner.getOldServerPinger().ping(ServerListEntryNormalPack.this.server);
                    }
                    catch (UnknownHostException var2)
                    {
                        ServerListEntryNormalPack.this.server.pingToServer = -1L;
                        ServerListEntryNormalPack.this.server.serverMOTD = TextFormatting.DARK_RED + I18n.format("multiplayer.status.cannot_resolve");
                    }
                    catch (Exception var3)
                    {
                        ServerListEntryNormalPack.this.server.pingToServer = -1L;
                        ServerListEntryNormalPack.this.server.serverMOTD = TextFormatting.DARK_RED + I18n.format("multiplayer.status.cannot_connect");
                    }
                }
            });
        }
        ...
        String tooltip = enhanceServerListEntry(this, this.server, x, listWidth, y, i1, j1);
        if (tooltip != null)
        {
    ...
    @Nullable
    public String enhanceServerListEntry(ServerListEntryNormalPack serverListEntry, ServerData serverEntry, int x, int width, int y, int relativeMouseX, int relativeMouseY)
    {
        FMLClientHandler inst = net.minecraftforge.fml.client.FMLClientHandler.instance();
        Map<ServerData, ExtendedServerListData> serverDataTag;
        serverDataTag = ObfuscationReflectionHelper.getPrivateValue(FMLClientHandler.class, inst, "serverDataTag");
